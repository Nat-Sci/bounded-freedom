...
A link here means **influence or interoperability**, not bundled code, endorsement, or automatic trust. The v0.4.0 Astra Edition Skills are an original synthesis. Third-party repositories are not vendored or installed by default.
...
| Method plane | What disciplined process should this task follow? | Owned here: six research and four mathematical Skills loaded on demand |
...
The optional Codex proxy adapter was prompted by Goat Yang's [reconnecting diagnosis](https://goatyang.com/ai-practice/agents/chatgpt-codex/codex-reconnecting.html). BoundedFreedom adopts only the smallest verified idea: when HTTPS is reachable, WebSocket times out, and an active macOS proxy is not inherited, explicitly import that system proxy into a local managed `.env` block. The repository does not copy an endpoint, enable TUN mode, or select an HTTP-only provider by default; current OpenAI configuration documentation and local diagnostics remain the authority for compatibility.
...
### Mathematical methods and optional runtimes

| Source | What it contributes | Boundary in BoundedFreedom |
| --- | --- | --- |
| [Stan User's Guide](https://mc-stan.org/docs/stan-users-guide/index.html) | Statistical model construction, dependence, missingness, diagnostics, and generated-quantity patterns | Informs `statistical-model-analysis`; Stan is an optional project runtime, not the method itself |
| [WHO Child Growth Standards](https://www.who.int/tools/child-growth-standards/standards) and [lifespan normative modeling guidance](https://elifesciences.org/articles/85082) | Age-conditioned reference distributions, developmental trajectories, centiles, and individual deviation | Inform the developmental/normative profile; cohort, instrument, reference population, and clinical meaning remain project-owned |
| [TRIPOD+AI](https://www.bmj.com/content/385/bmj-2023-078378), [PROBAST+AI](https://www.bmj.com/content/388/bmj-2024-082505), and [STARD-AI](https://www.nature.com/articles/s41591-025-03953-8) | Reporting, bias, applicability, and diagnostic-accuracy boundaries for prediction and AI studies | Inform the clinical-prediction profile; they do not authorize patient-level diagnosis or deployment |
| [PyTorch automatic differentiation notes](https://docs.pytorch.org/docs/stable/notes/autograd.html) | Executable gradient semantics, non-differentiable behavior, and autograd inspection | Optional evidence for network and loss audits; observed gradients are not a universal proof |
| [Theorem Proving in Lean 4](https://docs.lean-lang.org/theorem_proving_in_lean4/) | A future route for machine-checked definitions, statements, and proofs | Reference for `formal-proof-verification` Future Work only; no proof assistant or certification layer is currently bundled |
...
| --- | --- | --- | --- |
| `cost-efficient-orchestration` | [Agent Skills](https://agentskills.io/specification), [Codex subagents](https://learn.chatgpt.com/docs/agent-configuration/subagents), [Codex speed and Spark](https://learn.chatgpt.com/docs/agent-configuration/speed), [OpenAI Astra model guidance](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-astra), [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness), [SciAgent](https://github.com/smestern/sciagent), [open-coscientist](https://github.com/jataware/open-coscientist) | Chief accountability, progressive Skill loading, bounded execution contracts, separate general and code fast routes, compact phase handoffs, a balanced opportunity gate, evidence-gated frontier control, lane exit decisions, route receipts, per-unit model choice, independent review, a portable host boundary, and bounded research-lineage handoffs | **Deferred:** measured routing thresholds, authoritative cost telemetry, durable event state, and cross-project lineage synchronization. **Excluded:** task-wide luxury/basic profiles, forced model-share quotas, standing crews, recursive delegation, required graph infrastructure, and automatic merging of upstream changes |
...
| `mathematical-problem-mapping` | [research-paper-code-study](https://github.com/baizhanxu/research-paper-code-study-codex-skill), [Stan User's Guide](https://mc-stan.org/docs/stan-users-guide/index.html), and [PyTorch autograd notes](https://docs.pytorch.org/docs/stable/notes/autograd.html) | Artifact-first reconstruction; typed mathematical objects; reported, observed, derived, inferred, proposed, and unknown evidence states; claim-equation-code-data-output links; dimensional, shape, limiting-case, identifiability, and well-posedness checks; and one bounded next-method handoff | **Deferred:** deterministic equation and program-analysis adapters. **Excluded:** automatic extraction as truth, a universal ontology, a catch-all mathematics agent, and invented specifications without project authority |
| `statistical-model-analysis` | [Stan User's Guide](https://mc-stan.org/docs/stan-users-guide/index.html), [WHO Child Growth Standards](https://www.who.int/tools/child-growth-standards/standards), [lifespan normative modeling guidance](https://elifesciences.org/articles/85082), [TRIPOD+AI](https://www.bmj.com/content/385/bmj-2023-078378), [PROBAST+AI](https://www.bmj.com/content/388/bmj-2024-082505), and [STARD-AI](https://www.nature.com/articles/s41591-025-03953-8) | Explicit estimand and independence unit; reconstruction, design, and audit entries; cross-sectional, longitudinal, developmental/normative, and clinical-prediction profiles; missingness, diagnostics, uncertainty, calibration, sensitivity, and bounded interpretation | **Deferred:** project-specific R, Python, or Stan adapters and validated domain recipes. **Excluded:** automatic model selection, significance optimization, causal claims from prediction, and patient-level diagnosis or deployment authority |
| `neural-network-mathematical-analysis` | [PyTorch autograd notes](https://docs.pytorch.org/docs/stable/notes/autograd.html) and [Theorem Proving in Lean 4](https://docs.lean-lang.org/theorem_proving_in_lean4/) | Code-to-operator reconstruction; tensor, information, parameter, state, and gradient dependency maps; invariance, identifiability, stability, conditioning, counterexample, and redesign checks; and a proof-obligation ledger | **Deferred Future Work:** an independent `formal-proof-verification` Skill, formal translation contract, proof-assistant adapter, and kernel receipt after repeated strict-proof demand. **Excluded:** tests, symbolic output, or sampled numerical behavior as universal proof |
| `loss-objective-optimization` | [PyTorch autograd notes](https://docs.pytorch.org/docs/stable/notes/autograd.html) and established constrained and multi-objective optimization practice | Objective reconstruction; term, reduction, normalization, weight, mask, constraint, and gradient ledgers; scale, degeneracy, gaming, surrogate-alignment, and optimization-interaction checks; and falsifiable redesign contracts | **Deferred:** deterministic gradient, scale, and optimization-diagnostic adapters. **Excluded:** tuning for a preferred result, an objective-library dependency, silent architecture changes, and optimizer convergence as proof of scientific validity |
...
- Strict proof production and proof-assistant verification are explicit Future
  Work. Current mathematical Skills may record definitions, assumptions,
  informal derivations, counterexamples, and proof obligations, but must return
  `formal-proof-gap` when a machine-checked proof is required.
- Mathematical, statistical, literature, plotting, reproduction, and software
  runtimes do not yet have tested portable adapters.
...
The proof gap becomes an independent `formal-proof-verification` Skill only
after repeated real tasks justify a stable theorem language, formal translation
contract, proof-assistant environment, trusted-library boundary, and
machine-check receipt. These are candidates for evidence-driven additions,
not permission for existing Skills to expand silently.
...
3. **Load methods only when needed.** Six focused research Skills and four
   mathematical Skills replace a giant standing prompt or a permanent team of
   agents; the mathematical entry opens only one specialized leaf at a time.
4. **Route phases through a cost ladder.** Keep frontier capability on the hard control boundary, prefer balanced capability for stable coordinated work, and return narrow code work to the specialized fast-code route and other mechanical follow-up to the general fast lane through a compact checkpoint.
...
Snapshot reviewed: 2026-09-06. Links and project status may change.
