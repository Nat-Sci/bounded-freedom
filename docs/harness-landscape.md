...
| --- | --- | --- |
| [Codex](https://learn.chatgpt.com/docs/build-skills) | Layered `AGENTS.md`, open Agent Skills under `.agents/skills`, configurable subagents, permissions, worktrees, review, and a Responses-based provider seam | Reference adapter. The availability-aware task-kind matrix selects general fast, code fast, balanced, strong, or evidence-gated frontier work. `.codex/agents` contains only four unpinned canonical roles, requiring explicit model/effort launch controls and contract-specific permissions; every prospective Spark launch uses a fresh host-authoritative quota preflight when available, with fitting Luna/Terra alternatives for blocked code units |
