...
| --- | --- | --- |
| [Codex](https://learn.chatgpt.com/docs/build-skills) | Layered `AGENTS.md`, open Agent Skills under `.agents/skills`, configurable subagents, permissions, worktrees, review, and a Responses-based provider seam | Reference adapter. The canonical task-kind matrix selects general fast, code fast, balanced, strong, or evidence-gated frontier work. `.codex/agents` packages four compatible fixed defaults and four explicit-pair Routed adapters; the latter require advertised launch controls and preserve contract-specific permissions |
