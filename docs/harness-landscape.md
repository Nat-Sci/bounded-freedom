...
| --- | --- | --- |
| [Codex](https://learn.chatgpt.com/docs/build-skills) | Layered `AGENTS.md`, open Agent Skills under `.agents/skills`, configurable subagents, permissions, worktrees, review, and a Responses-based provider seam | Reference adapter. The task-kind matrix selects Luna for clear repeatable work, Terra for everyday reasoning and tool use, Sol for complex judgment, and evidence-gated Astra for hardest end-to-end work. `.codex/agents` contains only four unpinned canonical roles, requiring explicit model/effort launch controls and contract-specific permissions; unavailable pairs are unsupported, and absent quota telemetry remains unknown rather than blocked |
