...
assert_contains 'Builder and Coder do not execute overlapping write turns' "$orchestration_skill" "orchestration serializes write ownership"
assert_contains 'the old two-worker ceiling must not push discovery or implementation back into Chief' "$route_evals" "routing evaluations protect Chief offload with occupied Builder and Reviewer slots"
...
cp "$test_root/user-agents" "$test_root/unmanaged-prefix"
printf 'user-owned status note\n' >> "$test_root/unmanaged-prefix"
{ cat "$test_root/unmanaged-prefix"; awk -v begin="$begin_marker" '$0 == begin { keep = 1 } keep { print }' "$idempotent_root/.codex/AGENTS.md"; } > "$test_root/agents-with-prefix"
mv "$test_root/agents-with-prefix" "$idempotent_root/.codex/AGENTS.md"
"$installer" --host codex --target-root "$idempotent_root" --status > "$test_root/status-unmanaged.out"
assert_contains "Codex AGENTS.md: managed block present (current)" "$test_root/status-unmanaged.out" "unmanaged content alone keeps a managed block current"
cp "$idempotent_root/.codex/AGENTS.md" "$test_root/status-before"
...
assert_contains "Codex AGENTS.md: managed block present (source update available)" "$test_root/status-prefix.out" "status identifies managed payload drift"
assert_file_unchanged "$test_root/drift-before-status" "$idempotent_root/.codex/AGENTS.md" "status does not mutate a drifted managed block"
"$installer" --host codex --target-root "$idempotent_root" --update > "$test_root/status-restore.out"
"$installer" --host codex --target-root "$idempotent_root" --status > "$test_root/status-restored.out"
assert_contains "Codex AGENTS.md: managed block present (current)" "$test_root/status-restored.out" "update restores current managed payload"
sed "/^$begin_marker\$/,\$d" "$idempotent_root/.codex/AGENTS.md" > "$test_root/restored-prefix"
assert_file_unchanged "$test_root/unmanaged-prefix" "$test_root/restored-prefix" "update preserves unmanaged instruction content after drift"
